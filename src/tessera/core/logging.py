...
A single named logger (``tessera``) writes to the console; when an output
...
def configure_logging(output_dir: Path | None = None, level: int = logging.INFO) -> logging.Logger:
    """Configure and return the ``tessera`` logger.
    ...
    if output_dir is not None:
        ...
        run_log = logging.FileHandler(output_dir / "tessera.log")
