...
from ..core.errors import RecomFiError, UserInputError
from ..core.logging import configure_logging

app = typer.Typer(
    add_completion=False,
    no_args_is_help=True,
    help="RecomFi: identify recombination events in a query against a reference collection.",
)
...
def stage_errors(logger: logging.Logger) -> Iterator[None]:
    ...
    A :class:`RecomFiError` (expected, user-facing) is logged concisely. Any
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"recomfi {__version__}")
        raise typer.Exit()
